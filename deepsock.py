...
def execute_trade(symbol, signal_data, price_data):
    ...
    try:
        ...
        position_pct = float(suggested_pct) / 100.0
        ...
        return
    # --- 修改结束 ---
    print(f"使用计算出的数量: {amount} {symbol.split('/')[0]}")
    try:
        ...
        print(f"{symbol} 更新后持仓: {format_position_info(positions[symbol])}")
    except Exception as e:
...
def main():
    ...
    timeframe = next(iter(TRADE_CONFIG.values()))['timeframe']
    if timeframe == '1h':
